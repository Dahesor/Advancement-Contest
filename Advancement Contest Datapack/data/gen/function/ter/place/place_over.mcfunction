...
execute if score success gen matches 1 run data modify storage gen gen_log prepend value [{text:"\n[gen:place] 成功放置",color:"gray"},{storage:"gen",nbt:"place.id",color:"light_purple"}]
execute if score success gen matches 0 run data modify storage gen gen_log prepend value [{text:"\n[gen:place] 未能成功放置",color:"gray"},{storage:"gen",nbt:"place.id"}]
